...
#define IDC_STATIC 1000 // Создаем ResourceID для дочернего элемента
...
INT WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInst, LPSTR lpCmdLine, INT nCmdShow)
{
	...
	wClass.hIcon =(HICON) LoadImage(hInstance,"ICO\\camera.ico",IMAGE_ICON,LR_DEFAULTSIZE, LR_DEFAULTSIZE,LR_LOADFROMFILE);
	wClass.hIconSm =(HICON) LoadImage(hInstance,"ICO\\axe.ico",IMAGE_ICON,LR_DEFAULTSIZE, LR_DEFAULTSIZE,LR_LOADFROMFILE);
	//wClass.hCursor = LoadImage(NULL, IDC_ARROW);
	wClass.hCursor =(HCURSOR) LoadImage(hInstance, "CUR\\winblue\\bdrum.ani",IMAGE_CURSOR,LR_DEFAULTSIZE, LR_DEFAULTSIZE,LR_LOADFROMFILE );
